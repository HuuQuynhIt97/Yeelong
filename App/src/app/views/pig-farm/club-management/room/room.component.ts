...
export class RoomComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    L10n.load(load);
    this.subscriptions.push(this.serviceSite.currentSite.subscribe(site => {
      this.site = site
      this.loadData();

    }));
    this.subscriptions.push(this.serviceHall.currentHall.subscribe(hall => {
      this.hall = hall
      this.loadData();
    }));
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}Room/LoadData?siteGuid=${this.site?.guid || ''}&hallGuid=${this.hall?.guid || ''}`,
  ...
  async openModal(template, data = {} as Room) {
    ...
    if (!this.hall?.guid) {
      this.translate.get('Please select a Hall!').subscribe(data => {
        this.alertify.warning(data, true);
      })
      return;
    }
    ...
    if (data?.id > 0) {
      ...
      this.model.roomPhoto =  this.imagePath(item.roomPhoto);
  ...
  imagePathPreview(path,number) {
    let img = this.noImage
    if (path !== null ) {
      this.bookingService.checkImage(path).subscribe(res => {
        if(res === false) {
          // img = this.noImage
          this.listFileImage.push(this.noImage)
          return;
        }else {
          img = this.apiHost + path
          this.listFileImage.push(img)
          return;
        }
      })
    }else {
